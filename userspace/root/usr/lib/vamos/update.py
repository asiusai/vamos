...
def verify_system_contents(device: Path, expected_version: str) -> None:
  ...
  with mounted_read_only(device) as root:
    required = (
      ...
      root / "usr/share/vamos/EDLAA64.EFI",
  ...
  with tempfile.TemporaryDirectory(prefix="vamos-esp-") as mount_dir:
    ...
    try:
      required = (
        ...
        mount_path / ESP_EDL_APPLICATION,
      ...
      if required[5].read_bytes()[:4] != b"\xd0\r\xfe\xed":
...
def set_usb_mode(mode: str, *, reboot: bool = True) -> int:
  if os.geteuid() != 0:
    raise UpdateError("changing USB mode requires root privileges")
  if mode not in USB_MODES:
    raise UpdateError(f"unsupported USB mode: {mode}")
  previous = selected_boot_control()
  if previous is None:
    raise UpdateError("no valid redundant boot-control state found")
  ...
  values: dict[str, str | int] = {
    ...
    "root_a": str(previous["root_a"]),
    "root_b": str(previous["root_b"]),
    "edl_request": int(previous.get("edl_request", 0)),
    "usb_mode": mode,
  }
  payload = encode_boot_control(values)
  written = [_write_control_to_slot(slot, payload) for slot in ("a", "b")]
  if not any(written):
    raise UpdateError("no bootable ESP accepted the USB mode")
  os.sync()
  if reboot:
    run(["reboot", "-f"], capture=False)
  return int(values["generation"])
...
def _install_selector_on_slot(
  slot: str,
  selector: bytes,
  control: bytes,
  *,
  device: Path | None = None,
  edl_application: bytes | None = None,
) -> None:
  ...
  with mounted_esp(device, read_only=False) as esp:
    ...
    edl_path = esp / ESP_EDL_APPLICATION
    ...
    application = (
      edl_application if edl_application is not None
      else EDL_APPLICATION_SOURCE.read_bytes()
    )
    _atomic_write_bytes(edl_path, application)
    verify_arm64_efi(edl_path)
...
def prepare_disk_selector(active: str, target: str) -> int:
  ...
  with mounted_esp(target_device, read_only=True) as target_esp:
    ...
    edl_application = (target_esp / ESP_EDL_APPLICATION).read_bytes()
    verify_arm64_efi(target_esp / ESP_PRIMARY_LOADER)
    verify_arm64_efi(target_esp / ESP_KERNEL)
    verify_arm64_efi(target_esp / ESP_EDL_APPLICATION)
  ...
  _install_selector_on_slot(active, selector, control, edl_application=edl_application)
...
def main(argv: Sequence[str] | None = None) -> int:
  ...
  edl_parser = subparsers.add_parser("request-edl", help="request one-shot EDL through boot firmware")
  edl_parser.add_argument("--no-reboot", action="store_true")

  usb_parser = subparsers.add_parser("usb-mode", help="select the USB role for the next boot")
  usb_parser.add_argument("mode", choices=sorted(USB_MODES))
  usb_parser.add_argument("--no-reboot", action="store_true")
  ...
  try:
    ...
    with update_lock():
      if args.command == "install":
        ...
        set_usb_mode(args.mode, reboot=not args.no_reboot)
